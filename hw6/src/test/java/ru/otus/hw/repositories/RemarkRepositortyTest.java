...
@DataJpaTest
@Import({JpaRemarkRepository.class})
class RemarkRepositortyTest {
    ...
    void findById(Remark remark) {
        if (remark.getId() != updatedRemarkId && remark.getId() != deletedRemarkId) {
            var fRemark = jpaRemarkRepository.findById(remark.getId()).orElseThrow(() -> new EntityNotFoundException("Remark not found, id %d".formatted(remark.getId())));
    ...
    void insertRemark() {
        ...
        assertThat(returnedRemark).isNotNull()
                ...
                .isEqualTo(newRemark);
        Optional<Remark> checkRemark = jpaRemarkRepository.findById(returnedRemark.getId());
        assertThat(checkRemark)
                .isPresent()
                .get()
                .isEqualTo(returnedRemark);
        jpaRemarkRepository.deleteById(returnedRemark.getId());
    }
    ...
    void updateRemark() {
        ...
        assertThat(jpaRemarkRepository.findById(expectedRemark.getId()))
                .isPresent()
                .get()
                .isNotEqualTo(expectedRemark);

        var returnedRemark = jpaRemarkRepository.save(expectedRemark);
        assertThat(returnedRemark).isNotNull()
                .matches(remark -> remark.getId() > 0)
                .isEqualTo(expectedRemark);

        Remark bdReamrk = jpaRemarkRepository.findById(returnedRemark.getId()).get();
    ...
    @Test
    void deleteById() {
        assertThat(jpaRemarkRepository.findById(deletedRemarkId)).isPresent();
        var remark = jpaRemarkRepository.findById(deletedRemarkId);
        jpaRemarkRepository.deleteById(deletedRemarkId);
        Throwable exception = Assertions.assertThrows(EntityNotFoundException.class, () -> {
            jpaRemarkRepository.findById(deletedRemarkId).orElseThrow(() -> new EntityNotFoundException("Remark with id %d not found".formatted(deletedRemarkId)));});
